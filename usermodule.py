...
def help_():
    ...
    if command.lower().strip() == 'objective':
        print(
            'The objective of the game is to escape the subway station you may have to find keys or slay monsters to do so'
        )
    if command.lower().strip() == 'options':
        print(
            'Type move to move from room to room or type the same number room youre in to stay in that room \n Type examine to take a closer at the room youre in \n Type pick up to take items you want\n Type inventory to view all items in your inventory '
        )
    if command.lower().strip() == 'inventory':
        print(
            'This is where all the items youve collected are type open inventory to see the list of items you cureently have you may use items by typing use (chosen item)'
        )
...
def examine():
    if user_location == 2:
        print(
            ...
            'While taking in the room you notice that you can see your reflection in anything,there appears to be a foul smell coming from the west staircase'
            + Fore.RESET + Fore.YELLOW)
    elif user_location == 3:
        print(
            Fore.LIGHTMAGENTA_EX +
            'The rat appears to have a limp leg if you attack quick enough you might kill it'
            + Fore.RESET + Fore.YELLOW)
    elif user_location == 4:
        print(
            Fore.LIGHTMAGENTA_EX +
            'While taking in the subway cart you notice that you cant see your reflection in anything,there appears to be an umbrella and a bag on the floor'
            + Fore.RESET + Fore.YELLOW)
    elif user_location == 5:
        print(
            Fore.LIGHTMAGENTA_EX +
            'you stand on the train platform and notice its seem as though its been abandon quite a while you wonder if the rats took over?'
            + Fore.RESET + Fore.YELLOW)
...
def fight_scene_1():
    ...
    global game
    H = 10
    print(Fore.LIGHTMAGENTA_EX + 'A rat appears on the staircase' +
          Fore.RESET + Fore.YELLOW)
    print(Fore.LIGHTMAGENTA_EX +
          'The rat lunges at you but misses, what do you do' + Fore.RESET +
          Fore.YELLOW)
    command = input("\n >")
    if command == 'attack':
...
def bird_land_1_rooms():
    ...
    if user_location == 2:
        print(
            Fore.LIGHTMAGENTA_EX +
            'up the stairs,theres an exit to the west leading up to the street'
            + Fore.RESET + Fore.YELLOW)
        action()

    if user_location == 3:
        print(
            'Options attack the enemy,move back to the previous room,befriend the rat? '
        )
        action()
        fight_scene_1()

    if user_location == 4:
        print(Fore.LIGHTMAGENTA_EX + 'open subway cart ' + Fore.RESET +
              Fore.YELLOW)
        action()
    ...
    if user_location == 5:
        print(
            Fore.LIGHTMAGENTA_EX +
            'You awaken in what looks like an empty subway station there appears to be an EXIT sign to the north up the stairs an open Subway cart to your west and a locked bathroom to the south '
            + Fore.RESET + Fore.YELLOW)
        action()
    ...
    if user_location == 8:
        ...
        if command == 'use key' or 'key' and 'key1' in user_inv:
            print(
                Fore.LIGHTMAGENTA_EX +
                'The bathroom apears to have bare white walls with five stalls and five sink the mirrors seem to have been taken of the wall and only a black outline of what was once there '
                + Fore.RESET + Fore.YELLOW)
        else:
            print(Fore.LIGHTMAGENTA_EX + 'This room is locked' + Fore.RESET +
                  Fore.YELLOW)
...
def bird_land_2_rooms():
    ...
    if user_location == 7:
        print('NOT A ROOM TRY AGAIN')
    if user_location == 8:
        print(Fore.LIGHTMAGENTA_EX + ' locked bathroom' + Fore.RESET +
              Fore.YELLOW)
    if user_location == 9:
        print('NOT A ROOM TRY AGAIN')
